...
IMPORTANT: If you want to enable cluster-wide link:../concepts/concept_solidfire_concepts_security.html#encryption-at-rest-software[software encryption at rest], you must create the cluster using the link:../api/reference_element_api_createcluster.html[CreateCluster] API method instead and change the enableSoftwareEncryptionAtRest parameter to `true`. After software encryption at rest is enabled, it cannot be disabled on the cluster. Software encryption at rest is enabled by default for SolidFire eSDS clusters. Hardware-based encryption at rest can be link:../storage/task_system_manage_cluster_enable_and_disable_encryption_for_a_cluster.html[enabled and disabled] after cluster creation.
